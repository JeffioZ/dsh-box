...
插件安装、卸载和更新先形成内存中的“待应用”状态：手动操作允许继续批量修改，用户确认后再合并为一次服务重启；后台维护只在 RPC 能明确确认会话空闲时重启。每次手动增删在调用 `dsh plugin` 前保存原始 manifest 和单一未完成事务，旧事务未解决时拒绝覆盖；命令失败恢复原始 manifest，进程中断则在下次启动按包名或 manifest 差异定向收敛。用户主动卸载内置插件时，先持久化用户所有权，再清除事务标记。预置插件换包由清单中的 `replaces` 驱动：先清理仍由 DSHBox 管理的旧包，再安装后继包；旧包清理失败时不会安装后继包，用户主动卸载旧包的选择也会由后继包继承。重启前记录当前 dsh URL，服务恢复后回到原会话位置。
