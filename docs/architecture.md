...
首次配置不是独立启动线程：它是 `boot_inner` 中的等待点。`boot_once` 把内部错误映射为就绪、取消或失败三种业务结果，避免取消操作短暂显示成启动失败。
...
| `$DSH_HOME/cordis.patch.yml`、`$DSH_HOME/profiles/web/cordis.patch.yml` | dsh/用户 | dsh ≥0.1.7 的设置存储；只合并 locale/ui-theme entry；`llm-pi-ai` entry 只读（余额路由） |
| `$DSH_HOME/settings.yaml` | dsh/用户 | 旧版 dsh（<0.1.7）的设置存储，同上口径（新版首启会改名为 `.imported` 导入 patch 文档） |
...
插件安装、卸载和更新先形成内存中的“待应用”状态：手动操作允许继续批量修改，用户确认后再合并为一次服务重启；后台维护只在 RPC 能明确确认会话空闲时重启。插件变更在调用 `dsh plugin` 前保存批次事务与原始 manifest；CLI 在途拒绝新操作，CLI 完成但待服务验证时允许继续同一批次；单次命令失败只恢复该次操作前的 manifest，并保留此前成功项的恢复记录，进程中断则在下次启动按包名或 manifest 差异定向收敛。用户主动卸载内置插件时，先持久化用户所有权，再清除事务标记。预置插件换包由清单中的 `replaces` 驱动：先清理仍由 DSHBox 管理的旧包，再安装后继包；旧包清理失败时不会安装后继包，用户主动卸载旧包的选择也会由后继包继承。重启前记录当前 dsh URL，服务恢复后回到原会话位置。
...
这套方案降低构建复杂度，代价是没有模块打包与静态类型检查，因此 `npm run check` 会遍历全部受控文件，检查 JS 语法、HTML 本地引用、i18n 完整性、行尾、版本号同步、配置文件、文档链接、图标和右键注入契约。插件网络/进程操作和用量统计查询都放入阻塞任务池，避免占住 UI 的异步执行线程。
