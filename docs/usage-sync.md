...
| `src-tauri/src/net_guard.rs`（`guard_https_or_lan_http` / `read_json_capped`） | `lib/network.js` + `lib/accounts.js` 传输层 | 同一口径（https 任意主机 / http 仅私网放行、1 MiB 响应上限）；标题栏余额与用量账户共用 |
