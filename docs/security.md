...
- `dshd://` 只服务主窗口、当前可信 origin，并要求每次启动随机生成的令牌；令牌只经 `X-DSHd-Token` 请求头传递，不出现在任何 URL 中（避免进入页面 resource timing 缓冲区被同源脚本枚举），CORS 预检仅精确放行当前 dsh 来源；菜单完成初始化后会从页面全局对象删除该令牌。
