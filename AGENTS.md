...
- 只通过三条合法通道与 dsh 交互：① 注入 JS 到 dsh 页面（`MENU_INJECT` / `PAGE_INIT_SCRIPT`）；② 读写 `$DSH_HOME` 下的数据文件（只读会话日志；行级合并写 `settings.yaml` 的 `locale`/`ui-theme`/`llm-pi-ai` 段与 `.credentials.yaml` 的对应凭据）；③ 调用 `dsh` CLI（`dsh web`、`dsh plugin ...`）。
...
- `src-tauri/src/`：Rust 外壳，按职责分层（见 README「项目结构」）。`commands/` 只做 IPC 校验与转发。`usage/` 是「用量与余额」模块（聚合/日志解码/缓存/供应商/余额/订阅/状态栏实时统计/账户后台监测缓存）；第三方许可见 `THIRD_PARTY_NOTICES.md`。
...
- **注入脚本**：`PAGE_INIT_SCRIPT`（深色主题首帧预设）+ `locale::init_script()` 在窗口创建时作为 `initialization_script`；`MENU_INJECT` 是导航到 dsh 页面后 `eval` 的右键菜单注入（**对外部 URL 导航，initialization_script 不可靠，必须走 navigate 后的 eval**——见 `bootstrap.rs` 的 `on_navigation` 观察器）。
...
- **锁序约定**：需要多把锁时必须按 `lifecycle_guard` → `MARKET_PNPM_LOCK`（plugins）→ `RESTART_STATE` 的顺序获取，反向会死锁。服务重启（`updater::restart_service_locked`）与插件启动收敛都必须先与在途 `dsh plugin` CLI 互斥（`plugins::try_acquire_pnpm_lock`），否则停服会撞上 pnpm 半写状态。
- **后台任务**：依赖 dsh 就绪的周期任务统一走 `background.rs`（`service_gate` 门控 + `spawn_gated_periodic` runner），不自行拼 ownership/phase 条件；带跨轮状态或非标准节奏的任务手写循环时谓词仍须复用 `service_gate`。
- **进程树**：`processes.rs` 用 Windows Job / Unix 进程组守卫，退出时清理全部子进程，不留孤儿。
- **启动流程**：`dsh.rs::boot_loop` → `boot_once` → `boot_inner`。先在首选端口与官方 `3080` 探测外部 dsh（页面标记 + `host.describe`），再按用户选择接入或启动本地服务。本地端口顺序为“上次成功 → 首选 → `--port 0` 系统分配”，不扫描相邻端口；必须保留 `Child` 监控早退。服务归属为 managed/external/external-disconnected/none（未连接）四种，外部模式不得停止、重启、更新、维护插件或写本地 `$DSH_HOME` 来冒充外部配置。首次接入外部服务时要暂缓本地 onboarding，首次改用本地服务时再恢复，不得让本地配置永久跳过或让老用户重复引导。安装取消与服务选择都必须携带 `install_generation`。看门狗对托管服务自动恢复，对外部服务只报告断开。**新增启动页步骤应插入这套状态机，不要另起线程抢状态。**
- **内置插件市场**：`plugins::start_market_bootstrap` 在 dsh 就绪后按首次引导的明确选择安装 `resources/builtin-plugins.json` 中的插件；取消、跳过或缺少明确选择均不安装、不自动更新。此后每 24h（`market_last_check` 门控 + 常驻循环，首次同步延迟 90s）检查仍有内置身份的已安装包并后台升级。全部走 `dsh plugin` CLI，失败静默退避；pnpm virtual store 错位自动备份重建。手动增删插件在执行 CLI 前保存单一未完成事务和原始 manifest，旧事务未解决时拒绝开始新事务；失败恢复原始 manifest，进程中断则在下次启动定向收敛，用户卸载身份必须先落盘再清事务标记。多个变更合并为一次服务重启，自动维护等待会话空闲，重启后恢复原页面 URL。用户主动卸载会写 `market_user_removed_<pkg>`，插件页继续提供手动重装入口，但重装不清标记、不恢复内置身份；从未明确同意的同名手动安装包也不属于内置身份。换包用清单项的 `replaces` 声明完整前代链，迁移必须继承旧包的主动卸载标记，旧包清理失败时不得安装新包。强制下线加入 `MARKET_REMOVED`；当前包、替换前代与强制下线清单不得重叠。
...
- **提交前必查**：`cargo fmt`（及 `cargo clippy --all-targets -- -D warnings`、`cargo test --manifest-path src-tauri/Cargo.toml --all-targets`）。CI 的 Format/Clippy 步骤以 `-D warnings` 拦截任何格式与 lint 问题；**本地只验证了当前平台，非 Windows 平台的 cfg 门控死代码（如 `#[cfg(windows)]` 字段）只有全平台 CI 才能抓到**——功能改动尽量走 PR，让 CI 在合并前跑全平台。
- **发布 = 打 tag**：版本号同步到三处源文件（`src-tauri/Cargo.toml`、`package.json`、`src-tauri/tauri.conf.json`）及两份锁文件（`src-tauri/Cargo.lock`、`package-lock.json`，CI 有 version-check 校验）→ 提交 `chore: release x.y.z——…` → 打 `vx.y.z` tag 推送。CI 自动构建五平台（windows-x64/macos-arm64/macos-x64/linux-x64/linux-arm64）并发布 release，**不要手动传本地产物**（产物名与平台数量由 CI 保证）。发布日志正文来自仓库根 `RELEASE_NOTES.md`（CI 以 `body_path` 挂载，不自动生成），发布前更新为当前版本的内容。
