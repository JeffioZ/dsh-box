...
- **启动流程**：`dsh.rs::boot_loop` → `boot_once` → `boot_inner`。先在首选端口与官方 `3080` 探测外部 dsh（页面标记 + `host.describe`），再按用户选择接入或启动本地服务。本地端口顺序为“上次成功 → 首选 → `--port 0` 系统分配”，不扫描相邻端口；必须保留 `Child` 监控早退。服务归属为 managed/external/external-disconnected，外部模式不得停止、重启、更新、维护插件或写本地 `$DSH_HOME` 来冒充外部配置。首次接入外部服务时要暂缓本地 onboarding，首次改用本地服务时再恢复，不得让本地配置永久跳过或让老用户重复引导。安装取消与服务选择都必须携带 `install_generation`。看门狗对托管服务自动恢复，对外部服务只报告断开。**新增启动页步骤应插入这套状态机，不要另起线程抢状态。**
