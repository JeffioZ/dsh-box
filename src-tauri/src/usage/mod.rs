...
pub(crate) use live::{session_activity, set_visible_session};
...
mod tests {
    ...
    fn report_isolates_unreadable_generations_and_recovers_on_next_round() {
        ...
        // v5 为未知代次（v3/v4 已支持），模拟「日志格式较新」的会话
        std::fs::write(bad.join("session.v5.jsonl"), usage_line(1, 1, 5, 5)).unwrap();
        let partial = report(&config).unwrap();
        assert_eq!(partial.total.tokens, 120);
        assert_eq!(partial.unavailable_sessions, ["bad"]);
        std::fs::rename(bad.join("session.v5.jsonl"), bad.join("session.v4.jsonl")).unwrap();
