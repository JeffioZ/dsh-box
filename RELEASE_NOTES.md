...
- **Node 国内镜像兜底**：Node 包下载与版本索引检查均"官方直连失败 → 自动切换 npmmirror 镜像"，checksum 始终取自官方（镜像被篡改会被 SHA-256 拦截），国内网络首次启动不再卡下载
...
- **配置项**：`port` / `api_key` / `api_base` / `language` / `hide_tool_calls` / `hide_stats_line` / `hide_statusbar` / `dsh_update_channel`
...
- **应用本体自更新（Windows）**：GitHub Releases atom 查询（不受 API 限流影响），检查 → 下载并校验 → 确认后替换 exe（保留 `.old` 备份、失败回滚）→ 自动重启
...
- 单测覆盖版本解析、更新事务、主题/语言行级合并、release atom 解析、`--no-open` 版本判定等核心逻辑；CI 强制 fmt / clippy `-D warnings` / 单测（五平台）
